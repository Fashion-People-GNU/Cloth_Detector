...
CATEGORY_NAME = "yolov5_category"
COLOR_NAME = "yolov5_color"
LENGTH_NAME = "yolov5_length"
MATERIAL_NAME = "yolov5_material"
PRINT_NAME = "yolov5_print"
STYLE_NAME = "yolov5_style"

class_names = [COLOR_NAME, LENGTH_NAME, MATERIAL_NAME, PRINT_NAME, STYLE_NAME]

def image_crop(xyxy, im, gain=1.02, pad=10, square=False, BGR=False, save=True):
...
def run(
    ...
    results_dict = dict()
    detect_clothes_feature(agnostic_nms, augment, classes, conf_thres, data, device, dnn, half, hide_conf, hide_labels,
                           imgsz, iou_thres, line_thickness, max_det, nosave, save_crop, source, vid_stride, view_img,
                           visualize, weights, results_dict)
    return results_dict


def detect_clothes_feature(agnostic_nms, augment, classes, conf_thres, data, device, dnn, half, hide_conf, hide_labels,
                           imgsz, iou_thres, line_thickness, max_det, nosave, save_crop, source, vid_stride, view_img,
                           visualize, weights, results_dict):
    source = str(source)
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    for name in class_names:
                        label_sub = detect_clothes_feature_sub(agnostic_nms, augment, classes, conf_thres, data, device, dnn, half, hide_conf, hide_labels,
                           imgsz, iou_thres, line_thickness, max_det, nosave, save_crop, source, vid_stride, view_img,
...
def detect_clothes_feature_sub(agnostic_nms, augment, classes, conf_thres, data, device, dnn, half, hide_conf, hide_labels,
    ...
    dataset = LoadImages(img_path, img_size=imgsz, stride=stride, auto=pt, vid_stride=vid_stride)
    vid_path, vid_writer = [None] * bs, [None] * bs
    # Run inference
    model.warmup(imgsz=(1 if pt or model.triton else bs, 3, *imgsz))  # warmup
    seen, windows, dt = 0, [], (Profile(device=device), Profile(device=device), Profile(device=device))
...
if __name__ == "__main__":
    #weight_path = r"C:\Users\hong_\Desktop\수업\2024년 1학기\(usg)캡스톤종합설계\개발\전처리 모델\yolov5-master\clothes_detector\weights\yolov5_category\weights\best.pt"
    opt = parse_opt()
    main(opt)
